...
import { ConfidenceBar } from "../../reports/components/ConfidenceBar";
...
export default async function InitiativeReportPage({ params }: { params: Promise<{ petitionCode: string }> }) {
      ...
      <div className="card">
        ...
        <SimpleTable
          ...
          rows={snap.recent_batches.map((b) => ({
            id: (
              <Link href={`/reports/${encodeURIComponent(b.batch_id)}`} style={{ fontFamily: "var(--font-mono)", fontSize: "0.75rem" }}>
            ...
            link: <Link href={`/reports/${encodeURIComponent(b.batch_id)}`}>Detail</Link>,
          }))}
        />
      </div>
