...
import { webRunPlaceNonvoter } from "@/tools/voter-file-matcher/src/webReview";
...
export async function POST(request: Request, ctx: { params: Promise<{ batchId: string; rowNumber: string }> }) {
  ...
  if (note.length < 2) {
    return NextResponse.json({ error: "A note (reason) is required for nonvoter placement." }, { status: 400 });
  }
  loadVfmEnv();
  const pool = createPool();
  try {
    const result = await webRunPlaceNonvoter(pool, { batchId, rowNumber, note }, "REJECTED");
    return NextResponse.json({ ok: true, summary: result.summary });
  } catch (e) {
    const msg = e instanceof Error ? e.message : "Nonvoter placement failed.";
